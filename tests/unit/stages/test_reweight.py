...
def _make_organised_components(tmpdir, jets_per_flavour):
    ...
    config = {"files": {"train": files}, "num_global_objects": {"train": jets_per_flavour}}
...
class TestGetInputReaders:
    def test_caps_at_available_jets(self, tmp_path):
        ...
        readers, per_reader_num_global_objects = rw.get_input_readers()
        assert len(readers) == 2
        assert len(per_reader_num_global_objects) == 2
        # bjets has 50 < 100, should be capped at 50
        assert per_reader_num_global_objects[0] == 50
        # cjets has 200 >= 100, should use estimate
        assert per_reader_num_global_objects[1] == 100
    ...
    def test_all_above_estimate(self, tmp_path):
        ...
        _, per_reader_num_global_objects = rw.get_input_readers()
        assert per_reader_num_global_objects == [100, 100]
...
class TestCalculateWeightsStopIteration:
    ...
    def test_unequal_reader_lengths(self, tmp_path):
        """Readers with different num_global_objects don't crash the batch loop."""
        rw = _make_reweight_obj(
            tmp_path,
            jets_per_flavour={"bjets": 50, "cjets": 200},
