...
from upp.grid_split import get_output_ds_name
...
def create_meta_data(
    ...
    for split in ["train", "val", "test"]:
        ...
        for flavour in config.get_all_flavours():
            sel_containers = [
                Path(output_dir) / c for c in containers if split in c and flavour in c
            ]
            ...
            files_by_component[split][flavour] = [str(f) for f in vds]
...
def main():
    ...
    config = PreprocessingConfig.from_file(args.config, "train")
    ...
    for i, container in enumerate(split_containers):
        if not check_container_exists(container):
            print(f"Container {container} does not exist in Rucio. Please check the tag and user.")
            return
        else:
            print(f"[{i + 1}/{len(split_containers)}] Container {container} exists in Rucio.")
    output_dir = config.base_dir / "split-components"
